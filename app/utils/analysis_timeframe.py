...
def load_data():
    ...
    df = pd.DataFrame(rows, columns=["symbol", "timeframe", "trade_return"])
...
def run_analysis(df, name):
    ...
    for tf, g in grouped:
        ...
        print(f"{tf}: trades={len(g)}, winrate={winrate:.3f}, expectancy={expectancy:.4f}%")
...
def main():
    ...
    run_analysis(df, "ALL SYMBOLS")
    run_analysis(df_top50, "TOP50 ONLY")
