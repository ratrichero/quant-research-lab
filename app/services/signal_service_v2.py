...
from app.services.indicator_service import add_indicators, add_indicators_advanced, detect_regime, detect_regime_advanced
...
def calculate_score(df, pattern, cfg, symbol):
    ...
    # ================= COMPONENTS =================
    trend_score = momentum_score = volume_score = pattern_score = mtf_score = 0

    # TREND (Đã tối ưu cho Pullback / Reversal)
    ...
    if ema200 is not None and ema200 > 0:
        ...
        # khoảng cách % giữa giá và EMA200
        ...
        if direction == "LONG":
            ...
            # ✅ Pullback reversal zone (giá dưới EMA nhưng rất gần)
            elif abs_distance < 0.02:   # trong phạm vi 2%
                trend_score += 1

            # ✅ EMA50 alignment bonus (nhẹ)
            if ema50 is not None and ema50 > ema200:
                trend_score += 1
    ...
    if not pd.isna(rsi):
        if direction == "LONG":
            if rsi < 35: momentum_score += 2
            elif 35 <= rsi <= 45: momentum_score += 1
        else:
            if rsi > 65: momentum_score += 2
            elif 55 <= rsi <= 65: momentum_score += 1

    # VOLUME
    vol_ratio = None
    if not pd.isna(last.get("vol_ma")) and last["vol_ma"] > 0:
        vol_ratio = last["volume"] / last["vol_ma"]
        if vol_ratio > 2: volume_score += 2
        elif vol_ratio > cfg["VOLUME_MULTIPLIER"]: volume_score += 1

    # PATTERN QUALITY
    pattern_strength = {
        "Morning Star": 2, "Evening Star": 2,
    ...
    # ================= STRICT PENALTY =================
    strict_penalty, block_reason, ratios = strict_filters(df, cfg)
    
    # Log penalty vào components để debug
    components = {
    ...
    raw_score = trend_score + momentum_score + volume_score + pattern_score + mtf_score + strict_penalty
    ...
    """print(f"[DEBUG SCORE V2] T={trend_score} M={momentum_score} V={volume_score} "
          f"P={round(pattern_score,2)} MTF={mtf_score} PENALTY={strict_penalty} "
          f"→ TOTAL={final_score}")
    """
    return final_score, direction, components

def calculate_score_v3(df, pattern, cfg, symbol):
    ...
    mtf_norm = mtf_score / 1.75  # max approx
...
def run_market_scan():
    ...
    for symbol in symbols:
        ...
        try:
            ...
            # calculate_score v2 xaì hàm cũ nây
            ...
            features = build_features_from_row(last, score, direction)
